...
func cmdRewrite(args []string) (retErr error) {
	...
	fs.Usage = func() {
		fmt.Println("usage: livewire rewrite -in <in> -o <out> [edit options]")
		fmt.Println("\nApply static edits to a capture without replaying it.")
		printFlags(fs, flagIn, flagOut, "srcmac", "dstmac", "pnat", "portmap", "ttl", "fixcsum")
	}

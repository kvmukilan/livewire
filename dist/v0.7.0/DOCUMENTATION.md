# Livewire v0.7.0 Operator Guide
...
```powershell
Copy-Item .\dist\v0.7.0\livewire-0.7.0-windows-amd64.exe .\bin\livewire.exe
```
...
```bash
install -m 0755 ./dist/v0.7.0/livewire-0.7.0-linux-amd64 ./bin/livewire
```
...
Plain FTP control and data sessions are coordinated automatically by
`reproduce`. The planner recognizes `PASV`, `EPSV`, `PORT`, and `EPRT`, assigns
each negotiated data session to one control group by endpoint and chronology,
and blocks ambiguous or unmatched groups rather than replaying data alone.

Use `ftp-replay` for explicit or implicit FTPS, credential substitution, or
direct verification control:

```text
livewire ftp-replay -in issue.pcap -t ftp.example:21 \
  -set ftp.user=lab -set ftp.password=secret -verify strict
...
| `ftp-replay` | Coordinate FTP data sessions or reterminate explicit/implicit FTPS |
...
## 16. Deliberate v0.7.0 boundaries

...
These instructions download **0.9.0-rc.1**, a prerelease for validation.
...
Packet-based replay needs **Npcap** to send and receive packets, and
**WinDivert** to suppress unwanted resets during stateful TCP replay.
Socket-based application replay, including HTTP/TLS/FTP/SSH, needs neither driver.
...
Invoke-WebRequest -UseBasicParsing `
  -Uri https://github.com/kvmukilan/livewire/releases/download/v0.9.0-rc.1/livewire-0.9.0-rc.1-windows-amd64.zip `
  -OutFile livewire.zip
Invoke-WebRequest -UseBasicParsing `
  -Uri https://github.com/kvmukilan/livewire/releases/download/v0.9.0-rc.1/SHA256SUMS `
  -OutFile SHA256SUMS
$expected = (Select-String -Path .\SHA256SUMS -Pattern 'livewire-0.9.0-rc.1-windows-amd64.zip$').Line.Split(' ')[0]
...
The Windows publisher is intentionally unsigned; the checksum check above is
required before accepting an unknown-publisher warning.
...
Packet capture, injection, and stateful transport replay need **Administrator** PowerShell — sending raw
...
```bash
curl -fsSLO https://github.com/kvmukilan/livewire/releases/download/v0.9.0-rc.1/livewire-0.9.0-rc.1-linux-amd64
chmod +x livewire-0.9.0-rc.1-linux-amd64
sudo mv livewire-0.9.0-rc.1-linux-amd64 /usr/local/bin/livewire
livewire version
```

On a 64-bit ARM machine (Raspberry Pi 4/5, AWS Graviton) use
`livewire-0.9.0-rc.1-linux-arm64` instead. Native arm64 execution is not part of the
v0.9.0-rc.1 automated qualification; validate it on your target before relying on it.
...
```bash
livewire version    # prints: livewire 0.9.0-rc.1
...
official v0.9.0-rc.1 artifacts are reproducibly built with **Go 1.26.7**.

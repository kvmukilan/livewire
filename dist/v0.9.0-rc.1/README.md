...
Livewire reproduces recorded network exchanges against a live endpoint or a
device under test. The 0.9 release candidate uses **explicit replay intent**:
application, transport, wire, or automatic compatibility mode.

**Start here:** [Replay workflow](WORKFLOW.md) covers session selection, offline
preview, secure inputs, dashboard use, and compatibility with 0.7/0.8 commands.
The current version is **`0.9.0-rc.1`**, a prerelease for validation.
The stable release remains 0.8.0. Read the [release audit](RELEASE_AUDIT.md)
for reproduced regressions, passed checks, and remaining device/browser tests.
...
- [What 0.9 improves](#what-09-improves)
...
Choose `application` for fresh application sessions and response comparison,
`transport` for supported TCP/UDP behavior, or `wire` for captured frames without
response equivalence. `auto` keeps compatibility routing available explicitly.
The tool never silently changes an explicitly selected mode.

The default help includes `capture`, `check`, `reproduce`, `ifaces`, and `web`.
Use `livewire help commands` for the full catalog, `help examples` for workflows,
and `help <command>` for options. No-argument help succeeds on standard output.

`live <capture>` aliases `reproduce`, including flags before the capture.
`live -in <capture>` retains the historical TCP dry-run/on-wire engine. Scripts
omitting `--mode` retain automatic routing. Interactive reproduction asks for
intent when neither a mode nor an advanced profile was supplied.

The common flags remain `-in` (capture), `-i` (interface), `-t` (target), `-n`
(attempts), and `-details`. Existing aliases remain accepted. `--session` is
repeatable and `--dry-run` never opens a network connection.
...
- Explicit application, transport, and wire intent, with session selection and
  offline preview. Automatic detection informs the plan without overriding intent.
- Valid gzip/binary HTTP stays replayable; exclusions preserve packet accounting.
- A shared CLI/dashboard planner and fresh TLS/FTP/SSH executor.
- Dashboard preview invalidation, secure inputs, duplicate-start prevention,
  zero-gap attempts, current version, and accurate capture/report identity.
- Clear capture quality and readiness, plus the five everyday commands from 0.7.
- Patched SSH dependency (`x/crypto` v0.56.0); source builds now require Go 1.26.
  The supported release toolchains are 1.26.7 and 1.27.x.

Retained from 0.8:


- One protocol-aware front door for `reproduce <capture>` and `live <capture>`,
  including fresh TLS, FTPS, and SSH retermination and explicit-only wire mode.
- Exact secure-attempt outcome summaries: same, different, incomplete, or
  completed without comparison, with separate redacted attempt reports.
- Task help for protocols and diagnostic evidence, automatic-route readiness in
  `check`, typo suggestions, and successful no-argument help on standard output.
- Cross-platform no-replace artifact publication, numbered default evidence
  names, early output-collision checks, cancellable raw replay, and wire reports.
- One shared protocol-plan executor for the CLI and dashboard, eliminating
  divergent transport, semantic, FTP, and raw-wire dispatch implementations.
- Release publication now waits for compatibility, vulnerability, static,
  race, fuzz, coverage, deterministic-build, checksum, and provenance gates.

Carried over from v0.7.0:
...
- The v0.6 task-oriented help foundation, with five everyday
  commands; `livewire help commands` lists the supporting and compatibility tools.

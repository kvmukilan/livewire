...
Invoke-WebRequest -UseBasicParsing `
  -Uri https://github.com/kvmukilan/livewire/releases/download/v0.8.0/livewire-0.8.0-windows-amd64.zip `
  -OutFile livewire.zip
Invoke-WebRequest -UseBasicParsing `
  -Uri https://github.com/kvmukilan/livewire/releases/download/v0.8.0/SHA256SUMS `
  -OutFile SHA256SUMS
$expected = (Select-String -Path .\SHA256SUMS -Pattern 'livewire-0.8.0-windows-amd64.zip$').Line.Split(' ')[0]
...
```bash
curl -fsSLO https://github.com/kvmukilan/livewire/releases/download/v0.8.0/livewire-0.8.0-linux-amd64
chmod +x livewire-0.8.0-linux-amd64
sudo mv livewire-0.8.0-linux-amd64 /usr/local/bin/livewire
livewire version
```

On a 64-bit ARM machine (Raspberry Pi 4/5, AWS Graviton) use
`livewire-0.8.0-linux-arm64` instead. Native arm64 execution is not part of the
v0.8.0 automated qualification; validate it on your target before relying on it.
...
```bash
livewire version    # prints: livewire 0.8.0
...
Only needed if you are changing Livewire. Compatibility starts at **Go 1.25.14**;
official v0.8.0 artifacts are reproducibly built with **Go 1.26.7**.

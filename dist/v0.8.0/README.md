...
Livewire v0.8.0 is a protocol-adaptive PCAP replay platform for reproducing
...
- [The two primary commands](#the-two-primary-commands)
...
- [What v0.8.0 adds](#what-v080-adds)
...
Run `livewire` or `livewire help` for one task-based starting point. It keeps
advanced commands out of the way while making the next action explicit:

```text
livewire help examples       # copy-paste common workflows
livewire help troubleshoot   # recover from common problems
livewire help protocols      # automatic drivers and safe fallbacks
livewire help diagnose       # collect reproducible support evidence
livewire help commands       # list the complete command catalog
livewire help <command>      # usage and options for one command
```

Successful help is written to standard output and exits successfully, including
`livewire` with no arguments. Misspelled command names suggest the closest known
command without changing or running anything.

## The two primary commands

The default help surface has two actions. Both accept a positional capture and
use the same protocol-aware driver selection. Everything else is a supporting,
advanced, or compatibility tool listed by `livewire help commands`.

| Command | What it does |
|---|---|
| [`reproduce`](#reproduce) | guided, safe replay: detect, ask, open fresh sessions, verify, and report |
| [`live`](#live) | the same automatic routing, plus advanced controls and the historical `live -in` mode |

`livewire help <command>` shows one command's usage and options.
...
| Option | Means |
|---|---|
| `-in` | the capture file (`reproduce`, positional `live`, and `check` also take it bare) |
| `-i` | which network connection to use |
| `-t` | the device to talk to |
| `-n` | how many — attempts for a replay, packets for `capture` |
| `-o` | where to write |
| `-live` | compatibility `live -in` mode: actually send instead of simulating |
...
inspects every session before it opens an interface or connection and selects
the safest available driver automatically.

```sh
livewire reproduce issue.pcap -t 192.168.1.50
```

For ordinary TCP, UDP, and ICMP it asks for the device IP and, when needed, the
network connection. For TLS, FTPS, and SSH it opens a fresh application session
...
- The v0.6 task-oriented help foundation, now narrowed to the two primary

...
Windows/Linux NIC and DUT checks remain open; this is not the stable release.

Source builds now require Go 1.26 because the patched SSH dependency requires
it. Release artifacts use Go 1.26.7; CI also tests Go 1.27.x.

### Packaging and release gates

- The rc.1 tag did not publish: clean-checkout verification detected CRLF in
  embedded dashboard source. The release builder now normalizes embedded HTML
  and packaged text to LF, matching Git. rc.2 supersedes the failed candidate.
- Fuzz smoke uses 200,000 iterations per target with a two-minute watchdog to
  avoid Go's timed-fuzz shutdown race while keeping failures blocking.

### Fixed

- Recognized HTTP with binary/gzip responses and valid custom rule packs no
  longer lose to the opaque-payload heuristic.
- `-profile wire` and `--wire` use the same explicit wire path. Invalid profiles
  and unsupported route options fail before network activity.
- Inspection and execution share readiness decisions. Human output describes
  capture quality without claiming a replay-confidence percentage.
- Everyday help restores capture, inspection, interface discovery, and dashboard
  entry points. Historical command spellings remain callable.
- Dashboard preview invalidation, duplicate-start prevention, zero attempt gaps,
  current build version, and stale-capture detection were corrected.

### Added

- `--mode application|transport|wire|auto`, repeatable `--session`, and offline
  `--dry-run` on the primary replay workflow.
- Explicit exclusions preserve whole-capture packet accounting while results
  describe only the selected exchange. FTP control/data groups stay together;
  encrypted negotiation can be inspected with an explicitly supplied key log.
- CLI and dashboard share secure preparation/execution and readiness. The
  dashboard exposes conditional TLS/FTP/SSH inputs and expected/actual differences.
- Dashboard DUT previews now describe actual two-sided actor fidelity instead
  of advertising one-sided application adapters.
- Release candidate version is `0.9.0-rc.2`; NIC/DUT and browser acceptance
  checks remain requirements for stable promotion.

### Security and evidence

- Updated `golang.org/x/crypto` to v0.56.0 for GO-2026-6354 and GO-2026-6355
  (SSH connection deadlocks); the current scan has no reachable findings.
- Capture plans and reports hash the same byte stream that was loaded, so a
  replaced dashboard capture cannot be mislabeled in completed evidence.
- Partially specified SSH expectations and TLS runs with no compared responses
  cannot claim overall verified equivalence.
- Secure timing and truncated FTP data lanes fail in preview before execution.
- Added repeatable published-binary regression comparison and direct planner tests.

## [0.8.0] - 2026-08-31

### Highlights

- Makes `reproduce <capture>` and `live <capture>` the protocol-aware front
  door for plaintext, TLS, FTP/FTPS, SSH, ordinary transports, and explicit raw
  wire replay, with honest blockers instead of silent fidelity downgrades.
- Adds task-based help and diagnostic guidance aimed at first-time operators,
  plus automatic route/readiness output before a live packet is sent.
- Makes every generated artifact collision-safe and gives stateless wire replay
  a metadata evidence report, improving repeatability and support handoff.

### Added

- A unified, task-based `livewire help` hub with common examples,
  troubleshooting, a complete command catalog, per-command routing, and typo
  suggestions. Running `livewire` with no arguments opens the same hub and exits
  successfully.
- A shared protocol-aware primary orchestrator for `reproduce <capture>` and
  `live <capture>`. It routes plaintext adapters and ordinary transports through
  the existing planner, TLS through key-log decryption and fresh verified TLS,
  FTPS through the FTP coordinator, and SSH through fresh explicit operations.
- Interactive secure-input resolution with non-interactive errors that name the
  exact required flags. Adjacent key logs and `SSLKEYLOGFILE` are suggested but
  never silently consumed; unified SSH requires a pinned host key.
- An explicit `--wire` escape hatch. Automatic mode blocks unknown opaque
  sessions, DNP3 Secure Authentication, and unsupported security rather than
  transmitting ciphertext or silently falling back to frame injection.
- `livewire help protocols` explains automatic protocol routing and its required
  inputs; `livewire help diagnose` gives a short check, reproduce, and support
  bundle workflow.
- `check` reports the automatic route, required secure inputs, and blockers in
  both human-readable and JSON output.
- Metadata-only JSON evidence for raw `replay` / `--wire`, including capture
  digest, interface, timing mode, packet and byte counts, limitation, and error.
- A shared `internal/planexec` executor used by both the CLI and dashboard, with
  guarded dispatch for blocked, semantic, coordinated FTP, stateless, and
  stateful replay plans.

### Changed

- The default help surface now contains only the two primary commands,
  `reproduce` and `live`. All narrower commands remain available in the full
  catalog; `tls-replay`, `ftp-replay`, and `ssh-replay` are compatibility entry
  points for the same secure drivers.
- Positional `live <capture>` uses the same automatic orchestration as
  `reproduce`. Historical `live -in <capture>` parsing and behavior are retained
  for existing scripts.
- Successful top-level help now writes to standard output so Windows PowerShell
  does not display it as a native-command error.
- Automatic TLS routing is based on TLS record bytes rather than a conventional
  port number; opaque detection inspects both directions.
- Secure repeated runs classify the actual redacted retermination report from
  each attempt, preserve separate evidence files, and distinguish completed but
  unverified exchanges from real response differences.
- Release publication now waits for the full compatibility and quality matrices
  across Go 1.25.14, 1.26.7, and 1.27.x before artifacts are uploaded.

### Fixed

- Default reports and evidence files use a numbered unused name; explicit
  output paths fail before network activity if they already exist. Report and
  actual-output paths may not alias each other.
- Atomic file publication now has consistent no-replace behavior on Windows and
  Unix, so a late collision cannot overwrite an existing artifact.
- `--require-complete-capture` is enforced for TLS, FTPS, and SSH retermination;
  unified mode refuses to replay only one lane of a multi-session capture.
- Blank SSH expectations no longer count as a comparison, and the summary calls
  such completed exchanges `unverified` instead of `different`.
- Repeated secure replay honors `--stop-when-different` using the recorded
  outcome, not merely whether the runner returned an error.
- Raw replay responds to Ctrl-C/SIGTERM during timing waits and still emits its
  evidence report.

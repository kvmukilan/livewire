# Livewire v0.6.0 Operator Guide
...
- The one-sided `-t` value is an IP address. Each target port comes from the
...
.\livewire.exe live -in .\issue.pcap -i $Iface -t 192.168.1.50 -all
```
...
```powershell
Copy-Item .\dist\v0.6.0\livewire-0.6.0-windows-amd64.exe .\bin\livewire.exe
```
...
```bash
install -m 0755 ./dist/v0.6.0/livewire-0.6.0-linux-amd64 ./bin/livewire
```
...
`livewire` with no arguments lists only the everyday commands. `livewire help
--all` lists everything; `livewire help <command>` describes one.
...
|---|---|---|
| `-in` | the capture file | `reproduce`, `check`, `live`, `replay`, `rewrite`, `convert`, `lab`, `tls-replay`, `ssh-replay` |
| `-i` | which network connection to use | `reproduce`, `capture`, `live`, `replay` |
| `-t` | the device to talk to | `reproduce`, `live`, `tls-replay`, `ssh-replay`, `rstdrop` |
...
`reproduce` and `check` also accept the capture as a bare argument, in any
position relative to the options.
...
- Each attempt uses a fresh client port and ISN. An identical TCP four-tuple
...
- One report and one evidence PCAP cover the whole run. Each `sessions[]` and
...
## 16. Deliberate v0.6.0 boundaries
...
It refuses to build unless `go run ./cmd/livewire version` reports exactly that
version, which guards against a partial version bump. The version lives in four
places and all must agree: `cmd/livewire/main.go`, `internal/webui/webui.go`,
this script's default parameter, and `CHANGELOG.md`.

Pushing a `v*` tag runs `.github/workflows/release.yml`, which repeats the tests,
runs the same script, and publishes the artifacts to GitHub Releases using
